...
void alert();//aayush
int getBookCount();//tanish
void issuingBook(bklist b,char *id);//aayush
bklist bookauthSearch(char *z);//tanish
void funcall1();//aayush
void funcall2();//aayush
void issuedbokdet();//aayush
void alert_id(char *code);//aayush
void funcall3();//aayush
void funcall4();//aayush
char* libreq1();//aayush
char* vendor1();//aayush
void date_diff(char *idr);//AAYUSH
order *loaddatabase1();//tanish
void *booksearch1(char *z);//tanish
void *booksearchall(char *z);//tanish
void *booksearchid(char *z);//tanish
void statusfn(int sta,int ordid);//aayush
void *bookaddinfo(char *z);//tanish
void option();//aayush
void addabook(bklist1 *z);//tanish
int dumpis(cartstruct* a);//TANISH
bklist* issuebookv2(struct reqbook* headodr);//aayush
void headrvLoader();//tanish
void headisLoader();//tanish
void mainMenu();//aayush
void bookorder1();//aayush
void *bookorder(order *z);//aayush
bool approvingRequest(char *str1,char *str2);//aayush
void student(char *, char *);//Aayush
void studentPage(char *id);//Aayush
void staffPage(char *, char *);//Aayush
void staffMenu();//Aayush
void addMember();//tanish
void signIn();//tanish
bool approveRequest(int);//aayush
void deleteMember();//tanish
void uploadMembers();//tanish
char* teme();//AAYUSH
void passwordChange(int);//AAYUSH
void inventry();//AAYUSH
void loadMembers();//tanish
char *strupp(char *z);//tanish
char* libreq();//aayush
char* vendor();//aayush
int isValidDate(Date *validDate);//AAYUSH
void acpt_decline(int a);//AAYUSH
void pssrqst();//AAYUSH
struct reqbook* nthNode(int n);//tanish
int menucallking(int max,char *str);//TANISH
void showBookRequests();//Aayush
void returnbook(char *isome);//aayush
...
void issueBook(char *);//Aayush
void reserveBook(bklist *,char*);//aayush
void requestBook(char *);//aayush
bool loadCart(bklist *,char *);//tanish
int isReserved(bklist *book,char *id);//aayush
char* toUpperCase(char *s);//tanish
cartstruct* headLoader2(char *file1,char *file2);//TANISH
void cart(char *id);//TANISH
void headrq1Loader();//tanish
void headrq2Loader();//tanish
int dumpres(cartstruct* a);//TANISH
int dumpres1(cartstruct* a);//TANISH
void dumpreq();//tanish
void dumpreq1();//tanish
bklist* getBook(bklist *a);//tanish
...
struct reqbook *specificuserloader2(char *id);//tanish
...
void addMember()
{
  system("clear");
        ...
        printf("\nCreate a password(without spaces):\t");
        fgets(pass,50,stdin);
        int var=0;
        while(var==0)
        {
...
void signIn()
{
  ...
  i=i-1;
        char id[50],pass[50];
        printf("\nEnter your UserId(without spaces):\t");
        setvbuf(stdout, NULL, _IONBF, 0);
        fscanf(stdin,"%s", id);
        printf("\nEnter your Password(without spaces):\t");
        fscanf(stdin,"%s", pass);
...
void alert_id(char *coding)
{
        ...
        rewind(rsid);
        rewind(rstitle);
        while(fgets(line,1023,countrs)!=NULL)
        {
                Ml++;
        }
        rewind(countrs);
        ...
        for(int z=0; z<Nl; z++)
        {
                ...
                if(!cmp1)
                {
                    ...
                    fscanf(countrs,"%s", input1);
                    all = 0;
                    while(all<Ml && strcmp(input4,input2)!=0)
                    {
                            fscanf(countrs,"%s", input1);
                            fgets(input4,254,titlers);
                            input4[strcspn(input4, "\n")] = '\0';
                    ...
                    if(atoi(input1)!=0 && strcmp(input4,input2)==0)
                            { printf("\n --> %s",input4);
                              printf("\n  by %s",input5);
                              printf("\n is available now!\n");}
                    rewind(titlers);
...
void passwordChange(int i)//i=0->Student,i=1->Faculty,i=2->Staff
{
        ...
        file3=fopen("ps_rqt/id.txt","r"); if(fp2==NULL) printf("NULL");
        int Nl = 0,Ml= 0,ch1,var=0,var2=-1;
        ...
        while( fgets(line,1023,file3) != NULL)
        {
                Ml++;
        }
        rewind(fp2);
...
for(int z=0; z<Ml; z++)
        {
                ...
                fgets(input, 254, file3);
        }
while(cmp==1)
{
        printf("\n\nRequest Already Exists, Please wait for admin to take action");
        int xD=0;
                while (xD<3) {
                        sleep(1);
                        printf(".");
...
var2=-1;
  fscanf(fp2,"%s", input);
        ...
        printf("\nEnter the new password(without spaces):\t");
        setvbuf(stdin, NULL, _IONBF, 0);
...
char (*stringsplitter(char *z))[100] //Stringsplitter is a function which helps in searching book. When a string is passed in this function. It removes the whitespaces and put the different words in a 2D array (Array of strings). For example – if ‘hi this  is code’ is passed then  ‘hi’
//					       ‘this’
//					       ‘is’
//					       ‘code’ will be put in different rows of arrays.

// function which returns char(*)[100]
{
...
bklist bookSearch(char *z){ //This function helps in searching a string in the database and returning the book structure.
char (*buk_arr)[100];
char checkray[1000][3][100];// a 3D array which stores in which [1000] saves no of books whereas [3][100] stores array of strings
//for ex - if there is a book in which a string  is stored in first row then string will be in checkray[0][0] whereas another string of the same book will be in second row i.e checkray[0][1]
//but if there is one more book consisting 3 strings then these strings will be saved in checkray[1][0] , checkray[1][1], checkray[1][2]
buk_arr = stringsplitter(z); //stringsplitter will eliminate whitespace and save each word in different rows of array
bklist str_final;
char selected_bk[4];
bklist *a;
bklist *temp;
a = loaddatabase();// loads database into a linked list
temp = a;
int po=0;
for(;temp != NULL;/*po++*/)//accessing linked list
{
  for(int i=0;**(buk_arr+i)!='\0';i++)// this loop helps in searching words saved in buk_arr's rows in members of structures saved in linked list
  {
    char *sr1;
    sr1 = strstr(temp->auth,*(buk_arr+i)); // returns a string if buk_arr word is present in temp members
    char *sr2;
    sr2 = strstr(temp->Genre,*(buk_arr+i));
    char *sr3;
    sr3 = strstr(temp->Title,*(buk_arr+i));
    if(sr1 || sr2 || sr3)//if any one of sr1, sr2, sr3 exists
    {
      strcpy(checkray[po][0],temp->auth);// then members of structure for that temp will be copied in checkray where auth will be stored in 1st row of 'po'th book, similarly, Title and genre in 2nd and 3rd row
 ...
 if(po==0)// if no search result comes then function will return an empty structure
 {
 ...
 printf("S No.\n");
 for(int cnt1=0;cnt1<po;cnt1++)// prints the array(checkray) in which book details are stored
 {
   printf("       -------------------------------------------------------------------------------\n");
...
int sel;
sel = menucallking(po,selected_bk);// prompts user for a input (option which user wants to select)
strcpy(str_final.auth,checkray[sel-1][0]);//copies final book structure which is then returned
strcpy(str_final.Title,checkray[sel-1][1]);
strcpy(str_final.Genre,checkray[sel-1][2]);
return str_final;
}
int dumpdatabase(bklist* a)//this function takes in header of Bklist pointer type and dump the edited database back to the files and return the numbers of the nodes in the edited linked list.
{
bklist *temp; int cont;
FILE *aut,*tit,*gen,*iss,*cn_t;
aut = fopen("AUTHOR1.txt","w");//opening file handlers
tit = fopen("TITLE1.txt","w");
gen = fopen("Genre1.txt","w");
iss = fopen("issue_no1.txt","w");
cn_t = fopen("Count1.txt","w+");
for(temp = a->next, cont=0;;temp = temp->next)//dumping edited linked list to files from header
{
...
fclose(iss);
fclose(cn_t);
remove("AUTHOR.txt");//remove files
remove("TITLE.txt");
remove("Genre.txt");
remove("issue_no.txt");
remove("Count.txt");
rename("AUTHOR1.txt","AUTHOR.txt");//renaming files
rename("TITLE1.txt","TITLE.txt");
rename("Genre1.txt","Genre.txt");
rename("issue_no1.txt","issue_no.txt");
...
setvbuf(stdout, NULL, _IONBF, 0);
 ...
 void issuingBook(bklist b,char *id)
 {
   bklist *book;
 book=getBook(&b);
 if(book->count>0)
 {
   if(book->count>isReserved(book,id))
   {
     ...
     printf("3.Search more books by the author of this book\n\n");
     printf("4.Back to main menu\n\n");
     printf("Any invalid input will be taken as 4\n\n-->\t");
     int w=0;
     char el[10];
     setvbuf(stdin, NULL, _IONBF, 0);
     scanf("%s", el);
     ...
     if(w==3)
     {
       bklist c=bookauthSearch(b.auth);
       issuingBook(c,id);
     }
   }
   else
   {
     printf("\nSorry the book has been reserved and not available for issuing\n");
   }
 }
else
{
...
{system("clear");
 ...
 switch(w)
 {
   ...
   case 4: mainMenu();flag=false;break;
...
char *strupp(char *z)//returns passed string in uppercase
{
    for(int i=0;*(z+i)!='\0';i++)
    {
        *(z + i) = toupper(*(z+i));
...
cartstruct* cartissuelistloader(char *file, char *file1)//Works just like loaddatabase but loads the linked list of cartstruct type from the 2 file names passed in the function
{
 ...
 { commit = (cartstruct*)malloc(sizeof(cartstruct));
   ...
   temp->next = commit;//joining commit to the linked list
...
bklist bookcheck(int i)//takes in int parameter and and checks the book which has issue no == to the paramter passed
{
 bklist *temp;
 for(temp = head->next;temp != NULL;temp = temp->next)//access linked list
 {
   if(temp->issue == i)//checks temp->issue == parameter passed or not
   {
     return *temp;
   }
 }
 temp=NULL;
 return *temp;//returns the sturcture
}
...
int print(bklist *header, int i,char *z)// returns count of nodes in bklist linked list and also print the linked list in menu style
{
 bklist *temp;
 int j;
 printf("\n---------------------------------------------------\n");
...
int menucall(int max)//this function takes in the value of the maximum no(int max)  to be taken by the program.
//This function ensures that user types a number lying in Range [0,max].
...
bklist editbook(bklist* z, int i, bklist *a)//returns edited book structure
{
//so a is the header pointer passed from the loaddatabase function and z is
//the book info to be deleted or added in the database,
// and i defines the operation you wanna perform with the book info you passed -- 1 add book, 2 delete book(reduce count), 3 delete book permanently
...
temp = a->next;
prev = a;
for(;;)//accesing linked list
{
 str1 = strcmp(temp->auth,z->auth);//checks if book passed that is to be added or deleted == to the node of linked list
 str2 = strcmp(temp->Title,z->Title);
 str3 = strcmp(temp->Genre,z->Genre);
 if(str1 == 0 && str2 == 0 && str3 == 0)
 {
   if(i == 1)//increases count if book is availabel
   { cnt5 = 0;
     temp->count++;
     finedit = *temp;
     break;
   }
   else if(i == 2)//decreases count if book is available
   { cnt5 = 0;
     if(temp->count)
     {
     temp->count--;
   ...
   else if(i ==3)//deletes node permanentely
   {
     prev->next = temp->next;
     break;
   }
...
if (cnt5)//adds a book in end if book is not available
{
 issre = temp->issue;
 temp->next = z;
 z->issue = issre+1;
...
cartstruct editlist(bklist* z, int i, char* str, cartstruct *a)//works same like editbook but cartstruct type
{
//so a is the header pointer passed from the loaddatabase function and z is
//the book info to be deleted or added in the database,
...
bklist *nth_link(bklist *head, int i) //returns nth node of a linked list of bklist type
{
 bklist *book, *temp;
 int m;
...
int dumpres(cartstruct* a)////Works just like ‘Dumpdatabase’ but takes in the header of cartstruct pointer type and dump the edited database back to the files and return the numbers of the nodes in the edited linked list.
{
cartstruct *temp; int cont;
FILE *iss,*user;
user = fopen("tempReservedUserId.txt","w");
iss = fopen("tempReservedBooks.txt","w");//opening files
for(temp = a->next, cont=0;;temp = temp->next)//dump edited linked list in files
{
 fputs(temp->id,user);
 fprintf(iss,"%d\n", temp->issue);
 cont++;
...
fclose(iss);
fclose(user);
remove("ReservedUserId.txt");//deleting original files
remove("ReservedBooks.txt");
rename("tempReservedUserId.txt","ReservedUserId.txt");
rename("tempReservedBooks.txt","ReservedBooks.txt");// renaming to old files so that cycle can be repeated
return cont;
}
int dumpis(cartstruct* a)//Works just like ‘Dumpdatabase’ but takes in the header of cartstruct pointer type and dump the edited database back to the files
{
cartstruct *temp; int cont;
FILE *iss,*user;
user = fopen("tempCartUserId.txt","w");
iss = fopen("tempCartBooks.txt","w");
for(temp = a->next, cont=0;;temp = temp->next)//putting linked list back to files
{
 fputs(temp->id,user);
 fprintf(iss,"%d\n", temp->issue);
 cont++;
...
int dumpres1(cartstruct* a)////Works just like ‘Dumpdatabase’ but takes in the header of cartstruct pointer type and dump the edited database back to the files and return the numbers of the nodes in the edited linked list.
{
cartstruct *temp; int cont;
FILE *iss,*user;
...
void cart(char *id)
{
 ...
 //bklist  book1;
 //int cnt, cnt2, cnt3;
 char str1[] = "book you want to issue", str2[] = "book you have reserved till now";
 char str3[] = "Requested books that are available so far";
 ...
 strcpy(username,id);
  ...
  if(issuelisthead!=NULL)
  {
   ...
   reservelisthead = specificuserloader(username);
   count2=count1+0;
   ...
   orderlist = specificuserloader2(username);
   count3=count2+0;
   if(orderlist!=NULL)
   {
...
void staffMenu()
{
  ...
  while(flag)
  {
        ...
        printf("\t\t\t\t\t\t\t\t\t        ========================================================\n");
                ...
                case 6:mainMenu();flag=false;break;
...
char *cho;
        ...
        {
                char *vuo;
                vuo=vendor1();// a function to call vendors
                strcpy(c->vndr,vuo);
        ...
        {
                char *vuo;
                vuo=libreq1();//a function to call libraries
                strcpy(c->vndr,vuo);
        }
        else
        {
...
{
  {
    printf("Invalid option; Choose again ");
        ...
        rename("rep1.txt","IssuedBooks.txt");
        temp=1;
        rewind(issuedid);
        ...
        fclose(rep);
        fclose(issuedid);
        remove("IssuedUserId.txt");
        rename("rep2.txt","IssuedUserId.txt");
  ...
  switch(a)
  {
    ...
    case 1:
        ...
        printf("\t\t\t\t\t\t\t\t\t\t\t\t       ----------\n\n\n\n");
            ...
            printf("1. Search by Issue No. (select this option only if");
            printf(" you want to increase the count of a particular book)\n\n");
            printf("2. Search by Title or Genre or Author (select this option ");
            printf("if you want to add a new book to the database)\n\n");

            printf("Pease enter a valid option:\t");
            char *coi;
            coi=(char*)malloc(10);
            scanf("%s",coi);
            option4=atoi(coi);
            ...
            int seliss;
              ...
              if(&bookfinded==NULL)
              {
                printf("Sorry this issue no. doesn't exist.\nPlease try again\n");
                inventry();
              }
            }
    ...
    case 2:
        ...
        printf("\t\t\t\t\t\t\t\t\t\t\t\t\t  --------------\n\n\n\n");
            ...
            printf("1. Reduce Count of a particular book\n\n2. Delete a book permanently");
            int cnt11;
            scanf("%d",&cnt11);
            if(cnt11 == 1)
            {
...
gets(str);
strcpy(str,strupp(str));
bklist b=bookSearch(str);
bklist *book=getBook(&b);
return book->count;
}
void issuedbokdet()
{
        ...
        char *choose;
        printf("\n\nSelect one option (Any Invalid input will be taken as \"#\":\t");
        choose = (char*)malloc(100);
        scanf("%s",choose);
        int choice=atoi(choose);
        ...
        while(k==0)/*Here this while loop is used just to make user enter a valid input,if by mistake he/she entered an invalid input */
        {
          switch(atoi(ch1))
          {
            ...
            case 2: system("clear");
                    ...
                    while(l==0)
                    {/*Here this while loop is used just to make user enter a valid input,if by mistake he/she entered an invalid input */
                      switch(atoi(ch3))
                      {
                        case 1:
                                funcall4();
              ...
              case 3:
                    {
                      ...
                      while(m==0)/*Here this while loop is used just to make user enter a valid input,if by mistake he/she entered an invalid input */
                      {
                        switch(atoi(ch4))
                        {
                          case 1:{ /*Here I'm using regex to clear console screen*/
...
void funcall4()
{
        ...
        printf("\n\n#. Previous Menu\t\t0. Main Menu\n\n");
                      char el[10];
                      int choice;
                      printf("Any invalid choice will be taken as \"#\":\t\t");
  scanf("%s", el);
  choice=atoi(el);
  if(strcmp(el,"0")==0)
  {
...
void addabook(bklist1 *z) //this function takes in a book structure and then add it at the last of linked list
{
  bklist *current,*p;
  current=head->next;
  p=(bklist*)malloc(sizeof(bklist));
  while(current->next!=NULL) current=current->next;//accessing the linked and list now current->next points to NULL
  strcpy(p->auth,z->auth);
  strcpy(p->Title,z->Title);
  strcpy(p->Genre,z->Genre);
  p->issue=current->issue+1;
  p->count=atoi(z->qty);
  current->next=p;//current ->next points to p which is adress of the new node created thus p is added in the last.
  p->next=NULL;
  dumpdatabase(head);//this dumps the database back to the file using head (header of the linked list at the end of which book is added)now files consists of the details of the books added
}
...
if(daymax < daymin)
    ...
    if(didd>=15 && strcmp(input1,idi)==0)
    {
        printf("\n --> %s",input3);
        printf("\n  by %s",input4);
        printf("\n is overdue return now!\n");
    }
         fscanf(id,"%s",input1);
            fscanf(isno,"%s",input2);
            fscanf(dd,"%s",datestring5);       }
...
char* libreq1()
{
        int input=0,Nl=0;
        char line[1024];
        char vendrname[250],*vendor_select;
        FILE *vendr;
        printf("Order the book through :\n\n");
        vendr=fopen("liblist.txt","r");
        while( fgets(line,1023,vendr) != NULL)
